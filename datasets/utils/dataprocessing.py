...
class DataProcessing:
    ...
    def farthest_point_sample(pc, npoint): 
        ...
        """
        selectidx = np.zeros(npoint, dtype=np.int32)    # 采样点序号数组
        distance = np.ones(pc.shape[0], dtype=np.float32)* 1e10   # 初始化距离数组（取较大值）
        
        barycenter = np.sum(pc, axis=0)/(pc.shape[1])             # 计算重心坐标
        dist = np.sum((pc-barycenter)**2, -1)                     # 所有点到重心点的距离
        idx = np.argmax(dist)                                     # 将距离重心最远的点的序号

        for i in range(npoint):
            selectidx[i]=idx                                      # i=0 将距离重心最远的点作为第一个点；i>0 将距离数组中最大值对应的点作为后续点
            selectpc = pc[selectidx[i]]                                 # 取出这个最远点的xyz坐标
            dist = np.sum((pc - selectpc) ** 2, -1)                     # 计算点集中的所有点到这个最远点的欧式距离
            mask = dist < distance
            # print("mask %i : %s" % (i,mask))
            distance[mask] = dist[mask]                                     # 更新distance，记录样本中每个点距离所有已出现的采样点的最小距离
            # print("distance: ", distance)

            idx = np.argmax(distance)                         # 返回最远点索引
